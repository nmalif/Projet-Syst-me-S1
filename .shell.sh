...
imprimer()
{
  ...
  echo "Un fichier pdf a bien été généré, voulez-vous le voir ?"
  echo "Vous utiliseriez la souris pour parcourir le parcourir, CTRL+P pour l'imprimer et ALT+F4 pour quitter"
  while $c
    do
    read rep
    case $rep in
...
if [ $# -eq 0 ] # Si il ny pas dargument
  ...
  if [ $(wc -l temp | cut -d' ' -f 1) -eq 0 ] # sil ny a pas de .cpp ou .cc dans le repertoire alors on utilise le TEMPLATE
    then
    echo "Il n'y a pas de fichiers sources..."
    echo "... un template donc a été automatiquement crée !"
    maketemplate
    while $continuer
    do
    ...
    continuer=true
    mv template.$extension $fictoedit.$extension
    ...
    do
      ...
      read fictoedit
      if [ $(grep -cx $fictoedit temp) != 0 ] # Cela veut dire que lutilisateur a écrit un nom qui existait
        then
        echo
...
else # Sinon (dans ce cas là il y a plus dun argument) il y a erreur car il y a plusieurs fichiers sélectionnés !
...
do
  ...
  read reponse # On demande une réponse à lutilisateur tant que sa réponse est incorrecte
  if [ $reponse = "exit" ]
    then
    clear
